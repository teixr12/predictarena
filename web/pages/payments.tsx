  ...
  return (
    <Page trackPageView={'managrams page'}>
      <SEO
        ...
        description="See all credit transfers between users."
        url="/payments"
      />
      <Col>
        <span className={'text-primary-700 py-4 text-2xl'}>Credit Transfers</span>
        {payments ? (
...
const EmptyPaymentsState = () => (
  <Col className="border-ink-200 dark:border-ink-300 bg-canvas-50 items-center justify-center rounded-xl border border-dashed py-12">
    ...
    <span className="text-ink-500 mt-1 text-sm">
      Send credits to another user to get started
    </span>
  </Col>
)
    ...
    <Modal open={show} setOpen={setShow} size="sm">
      <Col className="bg-canvas-0 overflow-hidden rounded-xl shadow-xl">
        ...
        <div className="from-primary-600 to-primary-500 bg-gradient-to-r px-6 py-5">
          <div>
            <h2 className="text-lg font-semibold text-white">Send Credits</h2>
        ...
        <Col className="gap-5 p-6">
          ...
          <Col className="gap-2">
            ...
            <AmountInput
              ...
              onBlur={() => {
                if (amount && amount < 10 && !isAdmin) {
                  setError('Minimum amount is 10 credits')
                } else {
                  setError('')
                }
              }}
            />
            {!isAdmin && (
              <p className="text-ink-500 text-xs">Minimum: 10 credits</p>
            )}
          </Col>
  ...
  return (
    <Modal open={show} setOpen={setShow} size="sm">
      <Col className="bg-canvas-0 overflow-hidden rounded-xl shadow-xl">
        {/* Header */}
        <div className="from-primary-600 to-primary-500 bg-gradient-to-r px-6 py-5">
          <h2 className="text-lg font-semibold text-white">Receive Credits</h2>
          <p className="text-sm text-white/70">
            Share this QR code to receive credits from {user.name}
          </p>
        </div>
